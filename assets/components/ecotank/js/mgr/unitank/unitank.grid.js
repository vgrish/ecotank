ecotank.grid.UniTank = function (config) {
	config = config || {};
			...
			notifyDrop: function (dd, e, data) {
				...
				if (target != source) {
					...
					MODx.Ajax.request({
						...
						params: {
							action: config.action || 'mgr/unitank/sort',
							source: source,
							target: target
						},
						listeners: {
	...
	Ext.applyIf(config, {
		id: 'ecotank-grid-unitank',
		url: ecotank.config.connector_url,
		baseParams: {
			action: 'mgr/unitank/getlist',
			class: config.class || ''
		},
		save_action: 'mgr/unitank/updatefromgrid',
		autosave: true,
		...
		stateful: true,
		stateId: 'ecotank-grid-unitank-state'

	});
	ecotank.grid.UniTank.superclass.constructor.call(this, config);
	this.getStore().sortInfo = {
		field: 'rank',
		direction: 'ASC'
	};
};
Ext.extend(ecotank.grid.UniTank, MODx.grid.Grid, {
	windows: {},
			...
			listeners: {
				success: {
					fn: function (r) {
						var record = r.object;
						var w = MODx.load({
							xtype: 'ecotank-window-unitank-create',
							title: _('ecotank_action_update'),
							action: 'mgr/unitank/update',
							record: record,
							listeners: {
								success: {
									fn: this.refresh,
...
Ext.reg('ecotank-grid-unitank', ecotank.grid.UniTank);
