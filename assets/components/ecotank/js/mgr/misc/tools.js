...
ecotank.tools.getMenu = function (actions, grid, selected) {
    var menu = [];
...
ecotank.tools.renderActions = function (value, props, row) {
    var res = [];
    ...
    return String.format(
        '<ul class="ecotank-row-actions">{0}</ul>',
        res.join('')
    );
};
...
ecotank.tools.formatDate = function (string) {
    if (string && string != '0000-00-00 00:00:00' && string != '-1-11-30 00:00:00' && string != 0) {
        ...
        return date.strftime(MODx.config.ecotank_date_format || '%d.%m.%y <small>%H:%M</small>');
    }
    else {
        return '&nbsp;';
    }
...
ecotank.tools.renderReplace = function (value, replace, color) {
    if (!value) {
        replace = MODx.lang.ecotank_no_value || MODx.lang.ecotank_no;
    }
    if (!replace) {
        replace = value;
    }
    ...
    return String.format('<span class="ecotank-render-color" style="color: #{1}">{0}</span>', replace, color);
};


ecotank.tools.renderMoney = function (v, m) {
    ...
    if (!m && MODx.lang.ecotank_money_unit) {
        m = MODx.lang.ecotank_money_unit;
    }
    else if (!m && !MODx.lang.ecotank_money_unit) {
        m = '';
    }
    return whole + sub + ' ' + m;
};
...
ecotank.tools.renderLogo = function(value) {
    if (/(jpg|png|gif|jpeg)$/i.test(value)) {
        if (!/^\//.test(value)) {value = '/' + value;}
        return String.format('<img class="ecotank-render-img" src="{0}" />', value);
...
ecotank.tools.handleChecked = function (checkbox) {
