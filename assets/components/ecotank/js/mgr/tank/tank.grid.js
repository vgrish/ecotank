ecotank.grid.Tank = function (config) {
	config = config || {};
			...
			notifyDrop: function (dd, e, data) {
				...
				if (target != source) {
					...
					MODx.Ajax.request({
						...
						params: {
							action: config.action || 'mgr/tank/sort',
							source: source,
							target: target
						},
						listeners: {
	...
	Ext.applyIf(config, {
		id: 'ecotank-grid-tank',
		url: ecotank.config.connector_url,
		baseParams: {
			action: 'mgr/tank/getlist',
			class: config.class || ''
		},
		save_action: 'mgr/tank/updatefromgrid',
		autosave: true,
		...
		stateful: true,
		stateId: 'ecotank-grid-tank-state'

	});
	ecotank.grid.Tank.superclass.constructor.call(this, config);
	this.getStore().sortInfo = {
		field: 'rank',
		direction: 'ASC'
	};
};
Ext.extend(ecotank.grid.Tank, MODx.grid.Grid, {
	windows: {},
			...
			listeners: {
				success: {
					fn: function (r) {
						var record = r.object;
						var w = MODx.load({
							xtype: 'ecotank-window-tank-create',
							title: _('ecotank_action_update'),
							action: 'mgr/tank/update',
							record: record,
							listeners: {
								success: {
									fn: this.refresh,
...
Ext.reg('ecotank-grid-tank', ecotank.grid.Tank);
