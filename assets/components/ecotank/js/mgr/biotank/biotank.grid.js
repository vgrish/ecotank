ecotank.grid.BioTank = function (config) {
	config = config || {};
			...
			notifyDrop: function (dd, e, data) {
				...
				if (target != source) {
					...
					MODx.Ajax.request({
						...
						params: {
							action: config.action || 'mgr/biotank/sort',
							source: source,
							target: target
						},
						listeners: {
	...
	Ext.applyIf(config, {
		id: 'ecotank-grid-biotank',
		url: ecotank.config.connector_url,
		baseParams: {
			action: 'mgr/biotank/getlist',
			class: config.class || ''
		},
		save_action: 'mgr/biotank/updatefromgrid',
		autosave: true,
		...
		stateful: true,
		stateId: 'ecotank-grid-biotank-state'

	});
	ecotank.grid.BioTank.superclass.constructor.call(this, config);
	this.getStore().sortInfo = {
		field: 'rank',
		direction: 'ASC'
	};
};
Ext.extend(ecotank.grid.BioTank, MODx.grid.Grid, {
	windows: {},
			...
			listeners: {
				success: {
					fn: function (r) {
						var record = r.object;
						var w = MODx.load({
							xtype: 'ecotank-window-biotank-create',
							title: _('ecotank_action_update'),
							action: 'mgr/biotank/update',
							record: record,
							listeners: {
								success: {
									fn: this.refresh,
...
Ext.reg('ecotank-grid-biotank', ecotank.grid.BioTank);
