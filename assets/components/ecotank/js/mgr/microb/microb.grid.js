ecotank.grid.Microb = function (config) {
	config = config || {};
			...
			notifyDrop: function (dd, e, data) {
				...
				if (target != source) {
					...
					MODx.Ajax.request({
						...
						params: {
							action: config.action || 'mgr/microb/sort',
							source: source,
							target: target
						},
						listeners: {
	...
	Ext.applyIf(config, {
		id: 'ecotank-grid-microb',
		url: ecotank.config.connector_url,
		baseParams: {
			action: 'mgr/microb/getlist',
			class: config.class || ''
		},
		save_action: 'mgr/microb/updatefromgrid',
		autosave: true,
		...
		stateful: true,
		stateId: 'ecotank-grid-microb-state'

	});
	ecotank.grid.Microb.superclass.constructor.call(this, config);
	this.getStore().sortInfo = {
		field: 'rank',
		direction: 'ASC'
	};
};
Ext.extend(ecotank.grid.Microb, MODx.grid.Grid, {
	windows: {},
			...
			listeners: {
				success: {
					fn: function (r) {
						var record = r.object;
						var w = MODx.load({
							xtype: 'ecotank-window-microb-create',
							title: _('ecotank_action_update'),
							action: 'mgr/microb/update',
							record: record,
							listeners: {
								success: {
									fn: this.refresh,
...
Ext.reg('ecotank-grid-microb', ecotank.grid.Microb);
