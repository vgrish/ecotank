ecotank.grid.Pogreb = function (config) {
	config = config || {};
			...
			notifyDrop: function (dd, e, data) {
				...
				if (target != source) {
					...
					MODx.Ajax.request({
						...
						params: {
							action: config.action || 'mgr/pogreb/sort',
							source: source,
							target: target
						},
						listeners: {
	...
	Ext.applyIf(config, {
		id: 'ecotank-grid-pogreb',
		url: ecotank.config.connector_url,
		baseParams: {
			action: 'mgr/pogreb/getlist',
			class: config.class || ''
		},
		save_action: 'mgr/pogreb/updatefromgrid',
		autosave: true,
		...
		stateful: true,
		stateId: 'ecotank-grid-pogreb-state'

	});
	ecotank.grid.Pogreb.superclass.constructor.call(this, config);
	this.getStore().sortInfo = {
		field: 'rank',
		direction: 'ASC'
	};
};
Ext.extend(ecotank.grid.Pogreb, MODx.grid.Grid, {
	windows: {},
			...
			listeners: {
				success: {
					fn: function (r) {
						var record = r.object;
						var w = MODx.load({
							xtype: 'ecotank-window-pogreb-create',
							title: _('ecotank_action_update'),
							action: 'mgr/pogreb/update',
							record: record,
							listeners: {
								success: {
									fn: this.refresh,
...
Ext.reg('ecotank-grid-pogreb', ecotank.grid.Pogreb);
